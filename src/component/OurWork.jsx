  ...
  return (
    <section className="bg-white py-24 px-4 sm:px-6 lg:px-12">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl sm:text-4xl font-bold text-center text-[#18140F] mb-14">
          Our Work
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {works.map((work, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-[#f0f4ff] to-[#e0e7ff] rounded-md overflow-hidden shadow-md hover:shadow-xl transition duration-300 hover:-translate-y-1"
            >
              <img
                src={work.image}
                alt={work.title}
              ...
              />
              <div className="p-5">
                <h3 className="text-xl font-semibold text-[#18140F] mb-2">{work.title}</h3>
                <p className="text-gray-700 mb-4 text-sm leading-relaxed">{work.description}</p>
                <button className="bg-[#18140F] text-white text-sm px-4 py-2 rounded-md hover:bg-orange-500 transition-colors duration-200">
                  View Details
                </button>
              </div>
            </div>
